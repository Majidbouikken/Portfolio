...
const WorkCard: React.FC<WorkCardInterface> = ({ work }) => {
    ...
    return (
        <Link to={`/${work.title}`} className='group flex flex-col items-start gap-1 cursor-pointer'>
            <img key={thumbnail} src={work.thumbnail} alt={thumbnail} className='brightness-100 group-hover:opacity-50 transition-opacity duration-500 z-0' />
            <caption className='-mt-8 text-primary-500 font-bold'>{t(Category[work.category])}</caption>
